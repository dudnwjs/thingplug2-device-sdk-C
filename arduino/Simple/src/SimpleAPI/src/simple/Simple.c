...
int tpSimpleAddData(char* data, unsigned char length) {
    ...
    SKTtpDebugLog(SKT_LOG_LEVEL_INFO, "Content data : %s, length : %d", gContent->data, gContent->len);
#else
    SKTDebugPrint(SKT_LOG_LEVEL_INFO, "Content data : %s, length : %d", gContent->data, gContent->len);
#endif
    return TP_SDK_SUCCESS;
}
...
#ifdef SPT_DEBUG_ENABLE
    SKTtpDebugLog(SKT_LOG_LEVEL_INFO, "tpSimpleTelemetry\ntopic : %s\n%s", topic, gContent->data);
#else
    SKTDebugPrint(SKT_LOG_LEVEL_INFO, "tpSimpleTelemetry\ntopic : %s\n%s", topic, gContent->data);
#endif
        if(gContent) {
            if(gContent->data) {
                free(gContent->data);
...
#ifdef SPT_DEBUG_ENABLE
    SKTtpDebugLog(SKT_LOG_LEVEL_INFO, "tpSimpleAttribute\ntopic : %s\n%s", topic,  jsonData);
#else
    SKTDebugPrint(SKT_LOG_LEVEL_INFO, "tpSimpleAttribute\ntopic : %s\n%s", topic,  jsonData);
#endif
    rc = handleMQTTPublishMessageWithTopic(topic, jsonData);
    if(jsonData) free(jsonData);
    return rc;
...
#ifdef SPT_DEBUG_ENABLE
    SKTtpDebugLog(SKT_LOG_LEVEL_INFO, "tpSimpleResult\ntopic : %s\n%s", topic,  jsonData);
#else
    SKTDebugPrint(SKT_LOG_LEVEL_INFO, "tpSimpleResult\ntopic : %s\n%s", topic,  jsonData);
#endif
    rc = handleMQTTPublishMessageWithTopic(topic, jsonData);    
    if(jsonData) free(jsonData);
    return rc;
...
#ifdef SPT_DEBUG_ENABLE
    SKTtpDebugLog(SKT_LOG_LEVEL_INFO, "tpSimpleSubscribe\ntopic : %s\n%s", topic,  jsonData);
#else
    SKTDebugPrint(SKT_LOG_LEVEL_INFO, "tpSimpleSubscribe\ntopic : %s\n%s", topic,  jsonData);
#endif
    rc = handleMQTTPublishMessageWithTopic(topic, jsonData);
    if(jsonData) free(jsonData);
    return rc;
...
int tpSimpleInitialize(char* serviceID, char* deviceID) {
    ...
    mDeviceID = deviceID;
#ifdef SPT_DEBUG_ENABLE
    SKTtpDebugLog(SKT_LOG_LEVEL_INFO, "tpSimpleInitialize\nserviceID : %s, deviceID: %s", serviceID,  deviceID);
#else
    SKTDebugPrint(SKT_LOG_LEVEL_INFO, "tpSimpleInitialize\nserviceID : %s, deviceID: %s", serviceID,  deviceID);
#endif
    return TP_SDK_SUCCESS;
}
...
#ifdef SPT_DEBUG_ENABLE
    SKTtpDebugLog(SKT_LOG_LEVEL_INFO, "tpSimpleRawTelemetry\ntopic : %s\n%s", topic, telemetry);
#else
    SKTDebugPrint(SKT_LOG_LEVEL_INFO, "tpSimpleRawTelemetry\ntopic : %s\n%s", topic, telemetry);
#endif    
 ...
 */
 int tpSimpleRawAttribute(char* attribute, DATA_FORMAT format) {
    ...
    snprintf(topic, SIZE_TOPIC, topicBase, mServiceID, mDeviceID);    
#ifdef SPT_DEBUG_ENABLE
    SKTtpDebugLog(SKT_LOG_LEVEL_INFO, "tpSimpleRawAttribute\ntopic : %s\n%s", topic,  attribute);
#else
    SKTDebugPrint(SKT_LOG_LEVEL_INFO, "tpSimpleRawAttribute\ntopic : %s\n%s", topic,  attribute);
#endif
...
#ifdef SPT_DEBUG_ENABLE
    SKTtpDebugLog(SKT_LOG_LEVEL_INFO, "tpSimpleRawResult\ntopic : %s\n%s", topic,  result);
#else
    SKTDebugPrint(SKT_LOG_LEVEL_INFO, "tpSimpleRawResult\ntopic : %s\n%s", topic,  result);
#endif
