...
void MQTTMessageArrived(char* topic, char* msg, int msgLen) {
    ...
    if(rpcReqObject) {
        ...
        if(strncmp(cmd, "set_attr", strlen("set_attr")) == 0) {
            ...
            ArrayElement* arrayElement = (ArrayElement*) calloc(1, sizeof(ArrayElement));            
            arrayElement->capacity = 1;
            arrayElement->element = (Element *)calloc(1, sizeof(Element) * arrayElement->capacity);            
...
static void attribute(void) {
    ...
    item->value = (void *) MQTT_HOST;
    arrayElement->total++;

    item = &arrayElement->element[arrayElement->total];
    item->type = JSON_TYPE_RAW;
    item->name = "sysLocationLatitude";
    item->value = (void *) "35.1689766";
    arrayElement->total++;

    item = &arrayElement->element[arrayElement->total];
    item->type = JSON_TYPE_RAW;
    item->name = "sysLocationLongitude";
    item->value = (void *) "129.1338524";
    arrayElement->total++;
...
int start() {
    ...
    {
        char str[64];
        snprintf(str,64,"tpMQTTCreate result : %d", rc);
        SKTDebugPrint(LOG_LEVEL_INFO, str);
    }
    return rc;
}
